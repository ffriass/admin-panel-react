...
import { productColumns, productDetailColumns } from "../../datatable-source";
      ...
      <Grid container className="charts">
        <Grid xs={12} md={8}>
          <div className="datatable">
            ...
            <DataGrid style={{ height: 600, width: '100%' }}
              className="datagrid"
              rows={data || []}
        ...
        <Grid xs={12} md={4}>
          <div className="datatable">
            ...
            <div style={{  background:`${selectedDetail.length >0 ? 'rgba(0, 128, 0, 0.363)' : 'rgba(255, 0, 0, 0.05)'}`, height:40, display:'flex', justifyContent:'center', alignItems:'center'}}>
              {data?.find(x => x.id == selectedServiceId)?.name}
            </div>
            <DataGrid style={{ height: 565, width: '100%' }}
              className="datagrid"
              getRowHeight={() => 50} 
              rows={selectedDetail || []}
